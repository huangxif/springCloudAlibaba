...
CREATE TABLE IF NOT EXISTS  QQ_FRIENDS_TYPE (
    ...
    GROUP_TYPE  tinyint DEFAULT 1  COMMENT '',
    CREATE_TIME datetime NOT NULL COMMENT '添加时间',
    UPDATE_TIME datetime NOT NULL COMMENT '修改时间',
    PRIMARY KEY (FID),
    KEY FRIENDS_USER_ID_INDEX (USER_ID),
...
CREATE TABLE IF NOT EXISTS  QQ_FRIEND_SESSION (
    ...
    CID INT(11) NOT NULL  COMMENT '消息发起人',
    MESSAGE_StATUS  tinyint DEFAULT 1  COMMENT '消息状态:1未读,2已读,3撤回',
    LAST_MESSAGE  tinyint DEFAULT 0  COMMENT '是否最后一条消息:0:不是,1是',
    SESSION_TYPE  tinyint DEFAULT 0  COMMENT '是否群组:0:不是,1是',
    SESSION_STATUS  tinyint DEFAULT 0  COMMENT '会话状态:0:删除,1显示',
    PRIMARY KEY (SID),
    KEY RELATION_USER_ID (USER_ID),
    KEY RELATION_CID (CID)
    ) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT'消息列表';
